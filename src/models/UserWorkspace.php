...
class UserWorkspace
{
    ...
    {
        $queryStr = "SELECT * FROM UserWorkspace WHERE user_id = :user_id AND workspace_id = :workspace_id";
        $stmt = $this->pdo->prepare($queryStr);
    ...
    {
        if ($filterStr == "") {
            $queryStr = "SELECT UserWorkspace.*, Workspace.*, User.*, Role.*  FROM UserWorkspace
